...
loadFloor(){
    ...
    const preloaderTimeline = GSAP.timeline()
        for(let child in this.roomChildren){
            ...
            if(child !=='room'){
    preloaderTimeline.to(this.roomChildren[child]["position"],{y:this.roomChildren[child]["position"]["y"] +2,duration:0.15,ease: 'back.out'})
            }
        }

        preloaderTimeline.to("#hero",
        {opacity:1,duration:0.5}).to("#button-horizontal",
         { opacity: 1, duration: 0.5,onComplete: resolve })
        });
